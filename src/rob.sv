module rob #(parameter int NROB = 8,
  ...
  localparam logic [1:0] RT_REG = 2'd0;
  localparam logic [1:0] RT_ST  = 2'd1;
  ...
  always_comb begin
    ...
    commitStore = commitv && (rtype[head] == RT_ST);
    commitRd = rd[head];
    commitVal = val[head];
  ...
  always_ff @(posedge clk or posedge reset) begin
    ...
    if (reset) begin
      ...
      if (alloc0 && canAlloc0) begin
        busy[nt] <= 1'b1;
        ready[nt] <= 1'b0;
        rtype[nt] <= alloc0Store ? RT_ST : RT_REG;
        rd[nt] <= alloc0Rd;
        nt = nt + 1'b1;
        nc = nc + 1'b1;
      end
      ...
      if (alloc1 && canAlloc1) begin
        busy[nt] <= 1'b1;
        ready[nt] <= 1'b0;
        rtype[nt] <= alloc1Store ? RT_ST : RT_REG;
        rd[nt] <= alloc1Rd;
        nt = nt + 1'b1;
        nc = nc + 1'b1;
      end
